...
const assignSalesStage = inject('assignSalesStage');
const forwardMessage = inject('forwardMessage');
...
const onAssignSalesStage = (stage) => {
  if (assignSalesStage) assignSalesStage(stage, props.source.id);
  closeContextMenu();
};

const onForwardMessage = () => {
  if (forwardMessage) forwardMessage(props.source.id);
  closeContextMenu();
};
</script>
