...
class CannedResponse < ApplicationRecord
  ...
  def uazapi_quick_reply_ids
    additional_attributes&.dig('uazapi_quick_reply_ids') || {}
  end

  def uazapi_quick_reply_id(channel = nil)
    return uazapi_quick_reply_ids[channel.id.to_s] if channel.present?

    additional_attributes&.dig('uazapi_quick_reply_id') || uazapi_quick_reply_ids.values.first
  end

  def uazapi_message_type
    additional_attributes&.dig('uazapi_message_type').presence || 'text'
  end

  def uazapi_remote_file_url
    additional_attributes&.dig('uazapi_remote_file_url').presence
  end

  def uazapi_remote_doc_name
    additional_attributes&.dig('uazapi_remote_doc_name').presence
  end

  def uazapi_remote_content_type
    additional_attributes&.dig('uazapi_remote_content_type').presence
  end

  def uazapi_on_whatsapp?
    additional_attributes&.dig('uazapi_on_whatsapp') == true
  end
